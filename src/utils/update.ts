...
import { getPluginsCollection } from './db-sqlite'
...
export async function checkForUpdates() {
  ...
  for (const result of searchResults) {
    ...
    const currentVersion = existingVersions.get(packageName) as
      | string
      | undefined
  ...
  if (updates.length > 0) {
    ...
    updates.forEach((update) => {
      const currentVersion = existingVersions.get(update.package.name) as
        | string
        | undefined
